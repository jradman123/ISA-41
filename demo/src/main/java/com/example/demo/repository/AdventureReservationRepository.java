...
public interface AdventureReservationRepository extends JpaRepository<AdventureReservation,Long> {
    ...
    @Query(value = "select * from reservation a where instructor_id = ?3 and  reservation_start <= ?2 and  ?1 <= reservation_start and reservation_end <= ?2 and ?1 < reservation_end", nativeQuery = true)
    List<AdventureReservation> getAllInForInstructorLastWeek(LocalDateTime start, LocalDateTime end,int id);
}
